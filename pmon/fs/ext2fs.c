...
static int is_disk_valid(const unsigned char *buf)
{
...
static int pmbr_part_valid(const unsigned char *buf)
{
	int i;
	/* Make sure the MBR is a protective MBR(GPT) and not a normal MBR */
	for (i = 446; i < 510 && (buf[i + 4] != 0xee); i += 0x10) {
	}
	return (i < 510);
}
...
#if 0
		/* Only one partition will cause an error */
		if (!is_disk_valid(leadbuf)) {
			printf("disk error!\n");
			goto out;
		}
#endif
		if (pmbr_part_valid(leadbuf)) {
			/* Read the GPT header.  */
			...
			for (n = 8; n < header.maxpart; n++) {
				...
				/* sometimes we need load kernel at any partition */
				//if (memcmp(entry.type, gpt_boot_type, sizeof(gpt_boot_type)) == 0) {
					/* the start_sec is global variable */
