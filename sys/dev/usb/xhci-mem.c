				...
				struct xhci_segment *next, bool link_trbs)
{
	...
	prev->next = next;
	if (link_trbs) {
		val_64 = vtophys((uintptr_t)next->trbs);
...
static int xhci_scratchpad_alloc(struct xhci_ctrl *ctrl)
{
	...
	ctrl->dcbaa->dev_context_ptrs[0] =
		cpu_to_le64(vtophys((uintptr_t)scratchpad->sp_array));
	...
	for (i = 0; i < num_sp; i++) {
		...
		scratchpad->sp_array[i] = vtophys(cpu_to_le64(ptr));
	}
...
int xhci_alloc_virt_device(struct xhci_ctrl *ctrl, unsigned int slot_id)
{
	...
	byte_64 = vtophys((uintptr_t)(virt_dev->out_ctx->bytes));
					...
					struct xhci_hcor *hcor)
{
	...
	val_64 = vtophys((uintptr_t)ctrl->dcbaa);
	...
	trb_64 = vtophys((uintptr_t)ctrl->cmd_ring->first_seg->trbs);
	...
	for (val = 0, seg = ctrl->event_ring->first_seg;
			...
			val++) {
		trb_64 = 0;
		trb_64 = vtophys((uintptr_t)seg->trbs);
	...
	deq = vtophys((unsigned long)ctrl->event_ring->dequeue);
	...
	val_64 |= (vtophys((uintptr_t)(ctrl->erst.entries)) & ~ERST_PTR_MASK);
				     ...
				     struct usb_device *udev, int hop_portnr)
{
	...
	trb_64 = vtophys((uintptr_t)virt_dev->eps[0].ring->first_seg->trbs);
