...
static struct descriptor {
	...
	struct _usb_config_descriptor config;
	struct _usb_interface_descriptor interface;
	struct _usb_endpoint_descriptor endpoint;
 ...
 */
int xhci_usb_lowlevel_init(struct xhci *xhci, enum usb_init_type init)
{
	struct xhci_hccr *hccr = xhci->xhcic.hccr;
	struct xhci_hcor *hcor = xhci->xhcic.hcor;
	struct xhci_ctrl *ctrl = &xhci->xhcic;
	int ret;
 ...
 * @return none
 */
int xhci_usb_lowlevel_stop(struct xhci *xhci)
{
	struct xhci_ctrl *ctrl = &xhci->xhcic;
...
static int xhci_match(struct device *parent, void *match, void *aux)
{
	return 1;
}

extern struct hostcontroller host_controller;
static void xhci_attach(struct device *parent, struct device *self, void *aux)
{
	int result;

	struct xhci *xhci= (struct xhci*)self;
	struct confargs *cf = aux;
	struct xhci_hccr *hccr = xhci->xhcic.hccr = (struct xhci_hccr *)cf->ca_baseaddr;
	xhci->xhcic.hcor = (struct xhci_hcor *)(cf->ca_baseaddr
				+ HC_LENGTH(xhci_readl(&hccr->cr_capbase)));
	
	result = xhci_usb_lowlevel_init(xhci,USB_INIT_HOST);
	xhci->hc.uop = &xhci_usb_op;
	TAILQ_INSERT_TAIL(&host_controller, &xhci->hc, hc_list);

	xhci->rdev = usb_alloc_new_device(xhci);
...
struct cfattach xhci_ca = {
	.ca_devsize = sizeof(struct xhci),
	.ca_match = xhci_match,
	.ca_attach = xhci_attach,
};
...
static int pcixhci_match(struct device *parent, void *match, void *aux)
{
	struct pci_attach_args *pa = aux;
	if (PCI_CLASS(pa->pa_class) == PCI_CLASS_SERIALBUS &&
	    PCI_SUBCLASS(pa->pa_class) == PCI_SUBCLASS_SERIALBUS_USB) {
		if (((pa->pa_class >> 8) & 0xff) == 0x30) {
...
static void pcixhci_attach(struct device *parent, struct device *self, void *aux)
{
	int result;
	...
	pci_mem_find(pc, pa->pa_tag, 0x10, &membase, &memsize,
			&cachable);
	if (bus_space_map(memt, membase, memsize, 0, &sc_sh)) {
		printf("Cant map mem space\n");
		return;
	}
	cf.ca_baseaddr = sc_sh;
	cf.ca_reg = (void *)pa;

	config_found(self, &cf, NULL);
}



struct cfattach pcixhci_ca = {
	.ca_devsize = sizeof(struct device),
	.ca_match = pcixhci_match,
	.ca_attach = pcixhci_attach,
};

struct cfdriver pcixhci_cd = {
	.cd_devs = NULL,
	.cd_name = "pcixhci",
	.cd_class = DV_DULL,
};
