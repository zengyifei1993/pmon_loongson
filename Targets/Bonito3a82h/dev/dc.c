...
static void config_pll(unsigned long base, float pclk)
{
	unsigned volatile int *pllreg =(base == DC_BASE_ADDR)?0xbbd00230:0xbbd00238;
	unsigned int out;
	...
	*(volatile unsigned int *)(0xbbd0020c) |= (0x7 << 12);
	...
	for (i = 0; i < sizeof(vgamode) / sizeof(struct vga_struc); i++) {
		...
		if (vgamode[i].hr == FB_XSIZE && vgamode[i].vr == FB_YSIZE) {
			mode = i;
			config_pll(base, vgamode[i].pclk);
			break;
		}
	}
...
#ifdef DC_FB0
	config_pll(DC_BASE_ADDR, sysclk);
#endif

#ifdef DC_FB1
	config_pll(DC_BASE_ADDR_1, sysclk);
#endif
	return 0;
}
